 ...
 */
public class RouletteWheelNode extends FunctionNode {
	...
	public double evaluate(final double[] variables) {
		return getChild(0).evaluate(variables) != 0 ? getChild(1).evaluate(
				variables) : getChild(2).evaluate(variables);
	}
	...
	public String getInfixExpression() {
		...
		if (children.size() >= getMinChildren()) {
			infixExpression = "(IF " + getChild(0).getInfixExpression() + " THEN "
					+ getChild(1).getInfixExpression() + " ELSE "
	...
	public int getMaxChildren() {
		return RouletteWheelNode.nChildren;
	}
