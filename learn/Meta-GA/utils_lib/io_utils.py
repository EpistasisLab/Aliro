...
class Experiment:
    ...
    def build_paths(self, method_name):
        self.schema = basedir + '/lab/examples/' + \
...
def get_project_id(proj_info, basedir):
    proj_ids = [key for key in proj_info.keys() if proj_info[key]
                ['cwd'] == basedir[:-1]]
...
def get_args_profile(schema):
    ...
    for achoice, atype in zip(args_choice, args_type):
        if atype == 'int' or atype == 'float':
            number_list = [x for x in achoice if isinstance(
                x, int) or isinstance(x, float)]
            # None type or other options in int or float type
            arange = [x for x in achoice if not (
...
def get_input_file(_id, tmpdir):
    ...
    for file in files:
        response = http.request(
            'GET', 'http://' + lab_host + ':5080/api/v1/files/' + file['_id'])
