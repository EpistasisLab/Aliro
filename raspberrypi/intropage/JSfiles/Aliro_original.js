...
function remove_boundaries_on2d_plot() {
  ...
  svg.selectAll(".boundaries").transition().duration(1000).attr("opacity", "0");
}
...
function boxplot() {
  ...
  var rotate_project_on_x_axis = function (d) {
    ...
    if (keys.length < svg.selectAll("rect").size()) {
      ...
      svg.selectAll("rect").data(keys).exit().remove();
    ...
    svg
      ...
      .filter(function (d, i) {
        return d3.select(this).attr("class") == "class 0";
      })
      // .transition()
      // .delay(function(d,i){return(i*10)})
      // .duration(1000)
    ...
    svg
      ...
      // // get each rectangle's class attribute
      .filter(function (d, i) {
        return d3.select(this).attr("class") == "class 1";
      })
      // .transition()
      // .delay(function(d,i){return(i*10)})
      // .duration(1000)
    ...
    svg
      ...
      // // get each rectangle's class attribute
      .filter(function (d, i) {
        return d3.select(this).attr("class") == "class 2";
      })
      // .transition()
      // .delay(function(d,i){return(i*10)})
      // .duration(1000)
...
function boxplot_direct_from_y() {
  ...
  var barchart = function (d) {
    ...
    svg.select(".myYaxis").transition().duration(2000).attr("opacity", "1");
  ...
  var showpiechart_v1 = function (d) {
    ...
    var svg = d3
      .select("#dataviz_area_div_second")
      .append("svg")
  ...
  var showpiechart_v1_reset = function (d) {
    ...
    d3.select("#dataviz_area_div_svg_second").remove();
  ...
  var thickyaxis = function (d) {
    ...
    d3.select(".myYaxis").style("stroke-width", 3).style("opacity", 1);
  };

  var normalyaxis = function (d) {
    // make it thicker when mouse over on y axis class myYaxis
    d3.select(".myYaxis").style("stroke-width", 1).style("opacity", 1);
  };
  ...
  d3.select("#projectyaxia").on("click", project_y_axis);

  d3.select("#twoDPlot").on("click", twoDPlot);

  // d3.select("#rotateprojectxaxia")
  // .on("click", rotate_project_on_x_axis );

  d3.select("#rotateprojectxaxia").on("click", barchart);
  ...
  d3.select("#show_piechart").on("click", showpiechart_v1);
  ...
  d3.select("#fbbfirsttosecondbound").on("click", showboundariesSecondBound);
...
function decisiontree() {
  ...
  function update(source, n_labels) {
    ...
    nodeEnter
      ...
      .attr("xlink:href", function (d) {
        return (d.children || d._children || version2) &&
          ...
          : "http://fractalytics.io/wp-content/uploads/2019/05/num.png";
      })
      .attr("x", "-76")
      .attr("y", "-74")
      .attr("width", "20")
    ...
    nodeEnter
      ...
      .attr("x", function (d) {
        ...
        return d.children || d._children || version2
          ? -75 -
              (getTextWidth(
                d.size + " / " + ((d.size * 100) / TOTAL_SIZE).toFixed(0) + "%",
                10,
                "Verdana"
              ) +
                5.7 -
    ...
    nodeEnter
      ...
      .attr("x", function (d) {
        if (
          (d.children || d._children || version2) &&
          d.name.match(/(.*?)(<|>|=|!=|<=|>=|=<|=>)/i)[1].length <= 11
        ) {
          return d.children || d._children || version2
            ? -75 -
                (getTextWidth(
      ...
      .text(function (d) {
        if (d.children || d._children || version2) {
          return d.name.match(/(.*?)(<|>|=|!=|<=|>=|=<|=>)/i)[1].length <= 15
            ? d.name.match(/(.*?)(<|>|=|!=|<=|>=|=<|=>)/i)[1]
      ...
      nodeEnter
        ...
        .attr("width", function (d) {
          ...
          if (d.pred) {
            ...
            v =
              (parseInt(d.pred.split(",")[curr].match(/\d+/)[0]) * 133) /
              d.size;
          } else if (!d.children) {
            v = 40;
          }
        ...
        .attr("opacity", function (d) {
          ...
          if (d.pred) {
            ...
            val = (
              (parseInt(d.pred.split(",")[curr].match(/\d+/)[0]) * 100) /
              d.size
            ).toFixed(0);
    ...
    if (tree_branch)
      var link_stoke_scale = d3.scale
        .linear()
        .domain([0, 100])
        .range([1.5, default_strokeness]);
    else
      var link_stoke_scale = d3.scale.linear().domain([0, 100]).range([1.5, 8]);
    ...
    var tlink = svg.selectAll("text.tlink").data(tpaths, function (d) {
      return d.target.id;
    });
    ...
    tlink
      ...
      .attr("visibility", function (d) {
        return d.target.depth == 1 && !version2 ? "visible" : "hidden";
      })
      .attr("opacity", 0.5);
  ...
  function createLabels(labels) {
    ...
    for (i = 0; i < c_l.length; i++) {
      ...
      var legendG = svg1
        ...
        .attr("transform", function (d) {
          return (
            "translate(" + 0 + "," + (30 * i + Size / 33 + Size / 50) + ")"
...
function decisiontree_Version2() {
  ...
  // d3.json("./json/structure_iris_dc_2.json", function(error, flare) {
  d3.json("./json/structure_iris_dc_2.json", function (error, flare) {
    if (error) throw error;
